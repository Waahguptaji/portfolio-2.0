          ...
          <div className="flex-1 max-w-2xl p-6">
            <form className="flex flex-col gap-8 space-y-4">
              {fields.map(({ id, label, placeholder, bgColor }) => (
                <motion.div
                  key={id}
                  className="flex items-center gap-4 logo-font"
                  whileHover={{ scale: 1.02 }} // Slight scale on hover
                  transition={{ type: "spring", stiffness: 300 }}
                >
                  <label
                    className={`text-xl font-bold border-l-2 border-l-black dark:border-l-white ${bgColor} text-black dark:text-white px-3 py-1 shadow-sm`}
                  >
                    {label}
                  </label>
                  <input
                    type="text"
                    id={id}
                    className="border-b-2 border-black dark:border-gray-400 w-3/4 px-2 text-lg outline-none bg-transparent text-black dark:text-white placeholder-gray-500 dark:placeholder-gray-400 transition-all duration-200 focus:border-blue-500 focus:outline-none"
